...
function generateFilename() {
  ...
  hours = hours % 12 || 12;
  return `optel-analysis-${date}-${hours}-${minutes}${ampm}-utc.html`;
}
...
function mapFolderItemsToReports(items, folder, domain = null) {
  return items
    ...
    .map((item) => {
      ...
      // Extract date from filename (format: optel-analysis-YYYY-MM-DD-...)
      const dateMatch = filename.match(/optel-analysis-(\d{4}-\d{2}-\d{2})/);
...
/** Fetch reports from a domain folder */
