...
const waitForDashboard = (maxAttempts = 20, delay = 100) => {
  const checkDashboard = (attemptsLeft) => {
    if (document.querySelector('#facets')) return Promise.resolve(true);
    if (attemptsLeft <= 0) return Promise.resolve(false);
    return new Promise((resolve) => {
      setTimeout(() => resolve(checkDashboard(attemptsLeft - 1)), delay);
    });
  };
  return checkDashboard(maxAttempts);
};
...
function addBackToReportButton() {
  ...
  const handleClick = async () => {
    ...
    await showReportInline(reportPath, reportDate); // eslint-disable-line no-use-before-define
...
function renderReport(container, htmlContent, reportPath) {
...
async function showReportInline(reportPath, reportDate) {
  ['optel-detective-source-report', 'optel-detective-source-report-path', 'optel-detective-source-report-view']
    .forEach((k) => sessionStorage.removeItem(k));
...
export async function checkForSharedReport(getReports) {
  ...
  try {
    ...
    if (match) {
      const { markReportAsViewed, updateNotificationBadge } = await import('./report-state.js');
      markReportAsViewed(match.path);
      await updateNotificationBadge();
      showReportInline(match.path, reportDate);
    }
  } catch {
    // Silently fail if report can't be loaded
  }
}
...
window.addEventListener('popstate', async () => {
  ...
  setClaudeButtonVisibility(false);
  try {
    const { getSavedReports } = await import('./report-state.js');
    ...
    // Silently fail if report can't be loaded on popstate
  }
});
