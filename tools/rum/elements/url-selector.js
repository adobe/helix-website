...
export default class URLSelector extends HTMLElement {
  ...
  connectedCallback() {
    ...
    let iconDomain = input.value;
    if (input.value.endsWith(':all')) {
      iconDomain = input.value.split(':').slice(0, -1).join(':');
    }
    img.src = `https://www.google.com/s2/favicons?domain=${iconDomain}&sz=64`;
